...
def move(variables : SynthesisVariables, j : int, alpha : int, beta : int, delta : int) -> Formula_T:
    and_variables = []
    ...
    for i in range(alpha, beta+1):
        first_and = add_eq(variables.u(i+delta, j+1), variables.u(i,j))
        second_and = add_eq(variables.x(i+delta, j+1), variables.x(i,j))
