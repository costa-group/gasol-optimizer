...
def push_basic_encoding(j : int, theta_push : ThetaValue, sf : SynthesisFunctions, bs : int) -> AssertHard:
    ...
    right_term = add_and(add_leq(0, sf.a(j)), add_lt(sf.a(j), constants.int_limit), add_not(sf.u(bs - 1, j)), sf.u(0, j + 1),
...
def pop_uninterpreted_encoding(j : int, theta_pop : ThetaValue, sf : SynthesisFunctions, bs : int, o : Formula_T) -> AssertHard:
    left_term = add_eq(sf.t(j), theta_pop)
...
def non_comm_function_encoding(j : int, theta_f : ThetaValue, sf : SynthesisFunctions, bs : int, o : List[Formula_T],
    ...
    for i in range(0, n):
        right_term_first_and.append(add_and(sf.u(i,j), add_eq(sf.x(i,j), o[i])))
    for i in range(bs-n+1, bs):
        right_term_second_and.append(add_not(sf.u(i, j+1)))
    for i in range(bs+n-1, bs):
        right_term_third_and.append(add_not(sf.u(i, j)))
    right_term = add_and(add_and(*right_term_first_and), sf.u(0, j+1) , add_eq(sf.x(0,j+1), r),
                          move(sf, j, n, min(bs-2+n, bs-1), 1-n) , add_and(*right_term_second_and),
                         add_and(*right_term_third_and))
    return AssertHard(add_implies(left_term, right_term))


def comm_function_encoding(j : int, theta_f : ThetaValue, sf : SynthesisFunctions, bs : int, o0 : Formula_T, o1: Formula_T, r:Formula_T) -> AssertHard:
    left_term = add_eq(sf.t(j), theta_f)
    right_term = add_and(sf.u(0,j), sf.u(1,j),
                         add_or(add_and(add_eq(sf.x(0,j), o0), add_eq(sf.x(1,j), o1)), add_and(add_eq(sf.x(0,j), o1), add_eq(sf.x(1,j), o0))),
                         sf.u(0,j+1), add_eq(sf.x(0,j+1), r), move(sf, j, 2, bs-1, -1), add_not(sf.u(bs-1, j+1)))
    return AssertHard(add_implies(left_term, right_term))


def store_stack_function_encoding(j : int, theta_f : ThetaValue, sf : SynthesisFunctions, bs : int, o0 : Formula_T, o1 : Formula_T) -> AssertHard:
    left_term = add_eq(sf.t(j), theta_f)
...
# def encode_instruction_stack(instruction : EncodingInstruction, bounds : InstructionBounds, sf : Synthesisfariables) -> List[AssertHard]:
#     """
