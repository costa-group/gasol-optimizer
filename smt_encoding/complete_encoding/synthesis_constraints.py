...
def push_basic_encoding(sv : SynthesisVariables, j : int, bs : int, theta_push : ThetaValue) -> AssertHard:
...
def dupk_encoding(sv : SynthesisVariables, k : int, j : int, bs : int, theta_dupk : ThetaValue) -> AssertHard:
...
def swapk_encoding(sv : SynthesisVariables, k : int, j : int, bs : int, theta_swapk : ThetaValue) -> AssertHard:
...
def pop_encoding(sv : SynthesisVariables, j : int, bs : int, theta_pop : ThetaValue) -> AssertHard:
    left_term = add_eq(sv.t(j), theta_pop)
    right_term = add_and(sv.u(0,j), add_not(sv.u(bs-1, j+1)), move(sv, j,1,bs-1,-1))
    return AssertHard(add_implies(left_term, right_term))


def pop_uninterpreted_encoding(sv : SynthesisVariables, j : int, bs : int, o : int, theta_pop : ThetaValue) -> AssertHard:
    left_term = add_eq(sv.t(j), theta_pop)
    right_term = add_and(sv.u(0,j), add_eq(sv.x(0,j), o), add_not(sv.u(bs-1, j+1)), move(sv, j,1,bs-1,-1))
    return AssertHard(add_implies(left_term, right_term))


def nop_encoding(sv : SynthesisVariables, j : int, bs : int, theta_nop : ThetaValue) -> AssertHard:
    left_term = add_eq(sv.t(j), theta_nop)
    right_term = move(sv, j,0,bs-1,0)
    return AssertHard(add_implies(left_term, right_term))


def fromnop_encoding(sv : SynthesisVariables, b0 : int, initial_idx : int, theta_nop: ThetaValue) -> List[AssertHard]:
    constraints = []
